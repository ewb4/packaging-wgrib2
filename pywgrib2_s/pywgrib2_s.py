...
def inq(gfile,
    ...
    global grid_defn, sec0, sec1, sec2, sec3, sec4, sec5, sec6, sec7, sec8
...
# for weird grids nx=-1/0 ny=-1/0
    ...
    for sec, value in enumerate([Sec0, Sec1, Sec2, Sec3, Sec4, Sec5, Sec6, Sec7, Sec8]):
        if value != False:
            size = my_wgrib2.wgrib2_get_mem_buffer_size(sec+16)
            array = create_string_buffer(size)
            err = my_wgrib2.wgrib2_get_mem_buffer(array, size, sec+16)
            if err != 0:
                array = None
                if debug: print("inq error reading mem buffer ",sec+16,' or sec',sec)
        else:
            array=None
        if (sec == 0): sec0 = array
        if (sec == 1): sec1 = array
